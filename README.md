...
The public route is closed. That descriptor is generated by
`install-agent-plugin --execute` into the checkout it installs from, and is
not committed, so `claude plugin marketplace add AraneaDev/Knossos-MCP` fails
with `Marketplace file not found` instead of resolving. It has to fail there,
because the clone it would otherwise resolve to has no `vendor/`: its own
